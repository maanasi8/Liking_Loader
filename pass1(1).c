...
void main()
{
	...
	int i=0,progaddr,csaddr,add,len;
	FILE *fp1,*fp2;
	...
	while(strcmp(input,"END")!=0)
	{
		...
		{
			fscanf(fp1,"%s",name);
			strcpy(es[i].csname,name);
			strcpy(es[i].extsym,"  ");
			fscanf(fp1,"%x",&add);
			es[i].address=add+csaddr;
			fscanf(fp1,"%x",&len);
			es[i].length=len;
			fprintf(fp2,"%s ** %x %x\n",es[i].csname,es[i].address,es[i].length);
			i++;
		}
		else if(strcmp(input,"D")==0)
		{
			...
			{
				strcpy(es[i].csname,"  ");
				strcpy(es[i].extsym,input);
				fscanf(fp1,"%x",&add);
				es[i].address=add+csaddr;
				es[i].length=0;
				fprintf(fp2,"** %s %x\n",es[i].extsym,es[i].address);
				i++;
				fscanf(fp1,"%s",input);
			}
			csaddr=csaddr+len;
